...
sub  addnewemployee{
    my $sth = $dbh->prepare('insert into employee values(?,?,?)') or die "Couldn't prepare statement: " . $dbh->errstr;
...
sub deleteemployee($){
    $eid=shift;
    my $sth = $dbh->prepare('delete from employee where eid=?')
...
sub displaybyid($){
    $eid=shift;
    my $sth = $dbh->prepare('SELECT * FROM employee WHERE eid = ?') or die "Couldn't prepare statement: " . $dbh->errstr;
              # Read input from the user
          
    $sth->execute($eid) or die "Couldn't execute statement: " . $sth->errstr; # Execute the query
...
sub updateemployee($$){
    $eid=shift;
    $sal=shift;
    my $sth = $dbh->prepare('update employee set sal=? where eid=?') or die "Couldn't prepare statement: " . $dbh->errstr;			
    $sth->execute($sal,$eid);
}
